...
heatmap.plt <- function(x, size=1.5, lg=T, data=F, s=NULL, e=NULL, main=NULL){
  ...
  corrColorMatrix <- rgb(
    ...
    c(rev(seq(0, 1, length = K)), rep(0, K)), 
    rep(0, 2 * K)
    )
  ...
  title = ifelse(!is.null(main), main, "Heatmap for Correlation")
  ...
  for (i in 1:c.correlation ^ 2) 
    ...
    text(
      corr.coord[i, 1], 
      corr.coord[c.correlation ^ 2 + 1 - i, 2],
      round(X.corr[corr.coord[i,1], corr.coord[i,2]], digits=2),
      col = "white",
      cex=size
      )
}
heatmap.plt(x = c("O", "C", "M"), size = 1.5, data = T) # Test
